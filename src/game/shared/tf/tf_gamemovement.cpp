...
ConVar	tf_bunnyjump_max_speed_factor("tf_bunnyjump_max_speed_factor", "1.2", FCVAR_REPLICATED);
ConVar  tf_autojump("tf_autojump", "0", FCVAR_NOTIFY | FCVAR_REPLICATED, "Automatically jump while holding the jump button down");
ConVar  tf_duckjump("tf_duckjump", "0", FCVAR_REPLICATED, "Toggles jumping while ducked");
ConVar  tf_groundspeed_cap("tf_groundspeed_cap", "1", FCVAR_NOTIFY | FCVAR_REPLICATED, "Toggles the max speed cap imposed when a player is standing on the ground");
...
void CTFGameMovement::PreventBunnyJumping()
{
	...
	float maxscaledspeed = tf_bunnyjump_max_speed_factor.GetFloat() * player->m_flMaxspeed;
...
bool CTFGameMovement::CheckJumpButton()
{
	...
	if ( mv->m_nOldButtons & IN_JUMP && !tf_autojump.GetBool() )
		return false;
